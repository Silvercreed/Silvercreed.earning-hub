...
const taskCooldown=24*60*60*1000;
...
function loadDashboard(){
  ...
  userRef.on('value',snapshot=>{
    ...
    let diff=Date.now()-data.lastTask;
...
function claimTask(){
  ...
  userRef.transaction(user=>{
    if(user){user.earnedIncome+=income; user.totalBalance+=income; user.lastTask=Date.now();}
    return user;
  });
}
...
function showPage(page){
  ['dashboard','packages','deposit','withdraw'].forEach(p=>document.getElementById(p).style.display=(p===page)?'block':'none');
}
...
// ---- PACKAGES ----
const packages=[
{name:"Starter",price:10,roi:0.02},{name:"Basic",price:25,roi:0.02},{name:"Silver",price:50,roi:0.02},{name:"Gold",price:100,roi:0.02},
{name:"Platinum",price:150,roi:0.02},{name:"Diamond",price:200,roi:0.02},{name:"Elite",price:250,roi:0.02},{name:"VIP",price:300,roi:0.02},
...
function buyPackage(price,name){
  const userRef=db.ref('users/'+currentUser.uid);
  userRef.transaction(user=>{
    if(user && user.totalBalance>=price){user.totalBalance-=price; alert(name+" purchased!");} else{alert("Insufficient balance");}
    return user;
  });
}
...
function depositFunds(e){
  ...
  storageRef.put(file).then(snapshot=>{
    snapshot.ref.getDownloadURL().then(url=>{
      db.ref('deposits').push({user:currentUser.uid,amount:amount,screenshot:url,status:'pending'});
...
function withdrawFunds(e){
  ...
  userRef.transaction(user=>{
    if(user && user.totalBalance>=amount){
      ...
      db.ref('withdrawals').push({user:currentUser.uid,amount:amount,method:method,number:number,status:'pending'});
